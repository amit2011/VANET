		...
		if (Long.parseLong(Global.configurefile[j][5])==0){
			if (Double.parseDouble(Global.configurefile[j][2])>=30){
				if (Global.configurefile[j][7]=="-1"){
				    long det=(long) ((Double.parseDouble(Global.configurefile[j][2])-30)/2/0.1); // timer
				    if (det==0){
				     a=0;
				...
				if (Global.configurefile[j][7]=="-1"){
				    long det=(long) ((30-Double.parseDouble(Global.configurefile[j][2]))/2/0.1);
				    if (det==0){
				     a=0;
				     Global.configurefile[j][7]="-1";
					 Global.configurefile[j][5]="1";
			...
			if (k==0){
				...
				if (max==1){
					...
					if (Long.parseLong(Global.configurefile[j][6])==max){
						...
						double detx=det/2;
						if (Global.configurefile[j][7]=="-1"){
						    Global.configurefile[j][7]=String.format("%d", det);;//use this method
						    a=2;
						    return a;
						    }// the timer
						else{
